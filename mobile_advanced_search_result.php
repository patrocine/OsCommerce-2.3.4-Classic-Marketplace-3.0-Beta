...
// 	  tep_redirect(tep_mobile_link($origin_href, tep_get_all_get_params(), 'NONSSL', true, false));
...
// create column list
  ...
  $select_str = "select distinct " . $select_column_list . " m.manufacturers_id, p.products_id, p.products_porcentage, p.stock_nivel, p.products_status, p.time_entradasysalidas, p.time_mercancia_entregado_procesando, pd.products_description, pd.products_name, p.products_price, p.products_tax_class_id, IF(s.status, s.specials_new_products_price, NULL) as specials_new_products_price, IF(s.status, s.specials_new_products_price, p.products_price) as final_price ";
    ...
    for ($i=0, $n=sizeof($search_keywords); $i<$n; $i++ ) {
      switch ($search_keywords[$i]) {
        ...
        default:
          ...
          $where_str .= "(pd.products_description like '%" . tep_db_input($keyword) . "%' or pd.products_name like '%" . tep_db_input($keyword) . "%' or p.products_model like '%" . tep_db_input($keyword) . "%' or m.manufacturers_name like '%" . tep_db_input($keyword) . "%'";
