...
function get_products($id_group){
	...
	$products_price_query = tep_db_query("select products_description.products_name, products_stock.products_stock_real, products.products_quantity, products.products_price, products.products_model, products_to_categories.products_id, products_to_categories.categories_id FROM products, products_description, products_to_categories, products_stock where products_stock.products_id = products.products_id and products_stock.products_stock_real >= 1 and products.products_id = products_description.products_id ".$query." and products.products_id = products_to_categories.products_id and products_to_categories.categories_id = ".$id_group.  "  order by products_description.products_name ASC ");
	$x=0;
	while ($products_price = tep_db_fetch_array($products_price_query)){
		...
		if(SHOW_MODEL)
         ...
         $customers_porcentage_values = tep_db_query("select * from " . 'products_groups' . " where customers_group_id = '" . 	3 . "' and products_id = '" . 	$products_price['products_id'] . "' ");
       $customers_group = tep_db_fetch_array($customers_porcentage_values);

         $customers_porcentage_values = tep_db_query("select * from " . 'products_stock' . " where products_id = '" . 	$products_price['products_id'] . "' ");
       $customers_stock = tep_db_fetch_array($customers_porcentage_values);

		print "<tr bgcolor=\"".$col."\">".$model."<td width=\"1000\" class=\"productListing-data\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"" . tep_href_link(FILENAME_PRODUCT_INFO, "products_id=" . $products_price['products_id']) . "\">".$products_price['products_name']."   </a></td><td width=\"150\" align=\"right\" class=\"productListing-data\">".$products_price['products_stock_real']." Pcs</a></td>   </a></td><td width=\"150\" align=\"right\" class=\"productListing-data\">".$products_price['products_id']."</a></td><td width=\"150\" align=\"right\" class=\"productListing-data\">".$currencies->display_price($customers_group['customers_group_price'],TAX_INCREASE)."</td></tr>";
	}
}
