...
// class constructor
  ...
  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
   ...
   //  $wersdf_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id = '" . $order->products[$i]['id'] . "' and products_extrapeso_precio >= '" . 1 . "'");
 ...
 // $address_books_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id = '" . $products_idf . "' and products_extrapeso_precio >= '" . 1 . "'");
...
//}else{
  ...
  $autorizado_values = mysql_query("select * from " . TABLE_CUSTOMERS . " where customers_id = '" . $customer_id . "' and customers_payment_total >= '" . MAXIMO_ENVIO_CONTRAREEMBOLSO . "'");
 if   ($autorizado = mysql_fetch_array($autorizado_values)){


        //        $products_idf =  $a['products_id'];
