...
*/
  ...
  class az_categories {
    ...
    function az_categories() {
      $this->title = MODULE_BOXES_AZ_CATEGORIES_TITLE;
      $this->description = MODULE_BOXES_AZ_CATEGORIES_DESCRIPTION;

      if ( defined('MODULE_BOXES_AZ_CATEGORIES_STATUS') ) {
        $this->sort_order = MODULE_BOXES_AZ_CATEGORIES_SORT_ORDER;
        $this->enabled = (MODULE_BOXES_AZ_CATEGORIES_STATUS == 'True');

        $this->group = ((MODULE_BOXES_AZ_CATEGORIES_CONTENT_PLACEMENT == 'Left Column') ? 'boxes_column_left' : 'boxes_column_right');
      }
    }
    ...
    $categories_a = $tree[$counter]['name'];
     ...
     // }
      ...
      //$oldday1 = date("Y-m-d", $time1);
                  ...
                  if  ($tree[$counter]['productos'] == 0){
      $categoriestime_values = tep_db_query("select * from " . 'categories' . " where categories_productos = '" . 0 ."' and categories_id = '" . $tree[$counter]['id_categories'] ."'");

}else{

    $categoriestime_values = tep_db_query("select * from " . 'categories' . " where categories_time <= '" . $time1 ."' and categories_id = '" . $tree[$counter]['id_categories'] ."'");
}
...
if ($categoriestime = tep_db_fetch_array($categoriestime_values)){
  ...
  $status_categories = 0;
   ...
   $status_categories = 1;
     ...
     tep_db_perform(TABLE_CATEGORIES, $sql_data_array, 'update', "categories_id = '" . $tree[$counter]['id_categories'] . "'");
 ...
 $categories_string .= '&nbsp;(<label>' . $tree[$counter]['productos'] . '</label>)';
    ...
    function getData() {
      ...
      $categories_query = tep_db_query("select c.categories_id, c.categories_productos, cd.categories_name_http, cd.categories_name, cd.categories_name_suple, c.categories_image, c.parent_id from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '0' and c.categories_id = cd.categories_id and cd.language_id='" . (int)$languages_id ."' and cd.categories_status_visible='" . 1 ."' order by sort_order, cd.categories_name");
      ...
      $this->tep_show_category($first_element);
        ...
        $infobox->azSetBoxHeading(MODULE_BOXES_AZ_CATEGORIES_BOX_TITLE);
    ...
    function check() {
      return defined('MODULE_BOXES_AZ_CATEGORIES_STATUS');
    }

    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Enable Categories Module', 'MODULE_BOXES_AZ_CATEGORIES_STATUS', 'True', 'Do you want to add the module to your shop?', '6', '1', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Content Placement', 'MODULE_BOXES_AZ_CATEGORIES_CONTENT_PLACEMENT', 'Left Column', 'Should the module be loaded in the left or right column?', '6', '1', 'tep_cfg_select_option(array(\'Left Column\', \'Right Column\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sort Order', 'MODULE_BOXES_AZ_CATEGORIES_SORT_ORDER', '0', 'Sort order of display. Lowest is displayed first.', '6', '0', now())");
    }
    ...
    function keys() {
      return array('MODULE_BOXES_AZ_CATEGORIES_STATUS', 'MODULE_BOXES_AZ_CATEGORIES_CONTENT_PLACEMENT', 'MODULE_BOXES_AZ_CATEGORIES_SORT_ORDER');
    }
  }
