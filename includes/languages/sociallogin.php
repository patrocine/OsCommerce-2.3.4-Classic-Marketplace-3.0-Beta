  ...
  //$process = true;
   ...
   if(!tep_session_is_registered('customer_id')){
	 ...
	 //not login and show popup
			$msg = 'Please verify your email by clicking the confirmation link sent to you';
...
elseif (isset($_POST['LoginRadiusEmailClick']) && !empty($_POST['session'])){
	...
	$query = tep_db_query("select configuration_title, configuration_key, configuration_value, configuration_description from " . TABLE_CONFIGURATION . " where configuration_key = '" . mysql_real_escape_string($_POST['session']) . "'");
		...
		if((isset($_POST['gender']) && empty($_POST['gender']))||(isset($_POST['FirstName']) && empty($_POST['FirstName']))||(isset($_POST['LastName']) && empty($_POST['LastName']))||(isset($_POST['dob']) && empty($_POST['dob']))||(isset($_POST['address']) && empty($_POST['address']))||(isset($_POST['email']) && empty($_POST['email']))||(isset($_POST['city']) && empty($_POST['city']))||(isset($_POST['state']) && empty($_POST['state']))||(isset($_POST['postcode']) && empty($_POST['postcode']))||(isset($_POST['telephone']) && empty($_POST['telephone']))){echo $msg .= 'Please fill all the fields<br>'; $lrerror .= 'lremptyerror ';}
	...
	if(($lrsetting['allfields']==1)&&((empty($lrdata['postcode']))||(empty($lrdata['Email']))||(empty($lrdata['FirstName']))||(empty($lrdata['LastName']))||(empty($lrdata['gender']))||(empty($lrdata['dob']))||(empty($lrdata['address']))||(empty($lrdata['city']))||(empty($lrdata['state']))||(empty($lrdata['telephone'])))){$msg .= $lrsetting['popupfieldmessage']; $lrerror .= 'lrdeleteposterror123 ';}
 ...
 */
  function sociallogin_logging_existUser($lrdata) {
    ...
    // Defining global variables. 
	  ...
	  tep_db_query("update " . TABLE_CUSTOMERS_INFO . " set customers_info_date_of_last_logon = now(), customers_info_number_of_logons = customers_info_number_of_logons+1, password_reset_date = null where customers_info_id = '" . (int)$customer_id . "'");
...
function sociallogin_account_verification_mail($Email, $sociallogin_id, $verification_code){
			...
			$url_address =  (($request_type == 'SSL') ? HTTPS_SERVER : HTTP_SERVER) . DIR_WS_CATALOG; 
			 $txt = 'Please click on the following link or paste it in browser to verify your email:-';
 ...
 */
 ...
 function sociallogin_verification_link($first, $second){
	...
	if($check_number_row_selected < 1){
		...
		$msg = "Your email has been successfully verified. Now you can login to your account.";
	...
	var loginRadiusForm = document.getElementById('loginRadiusForm');
		...
		for(var i = 0; i < loginRadiusForm.elements.length; i++){
			...
			if(loginRadiusForm.elements[i].value.trim() == ""){
				loginRadiusErrorDiv.innerHTML = "<b>Please fill all the fields</b>";
			...
			if(loginRadiusForm.elements[i].name == "email"){
				...
				if(atPosition < 1 || dotPosition < atPosition+2 || dotPosition+2>=email.length){
					loginRadiusErrorDiv.innerHTML = "<b>Please enter a valid email address</b>";
					loginRadiusErrorDiv.style.backgroundColor = "#f6d9d0";
					loginRadiusErrorDiv.style.border = "1px solid #990000";
...
<?php
   ...
   $output .= '</div><form id="loginRadiusForm" onsubmit="return validateForm();" method="post" action=""><div align="right"><span style="padding-right:30px" class="inputRequirement">* Required information</span></div><div ';
   ...
   <span id="errormassages"><br />Ex: example@example.com</span></td></tr>';
   }
   if($lrsetting['allfields'] != 0){
   if(empty($lrdata['city']) || $lrdata['city'] =='unknown'){
   ...
   if(empty($lrdata['state'])){
   $output .= '<tr><td><b id="textmatter">State/Province<span id="errormassages">*</span></b></td><td><input type="text" name="state" value="'.(isset($_POST['state'])?htmlspecialchars($_POST['state']):'').'"'; 
   ...
   $output .= '<tr><td><b id="textmatter">Country<span id="errormassages">*</span></b></td><td>'; 	    
   ...
   $output .= '<center><input type="submit"  name="LoginRadiusEmailClick" value="Submit" class="inputbutton" onClick="loginRadiusPopupSubmit = true"/>';
   $output .= '&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" value="Cancel" class="inputbutton" name="cancel" onClick="loginRadiusPopupSubmit = false"/><input type="hidden" value="'.$lrdata['session'].'" name="session" id="session" /></center>';
   $output .= '</div></form></div></div></div>';
   return $output;
   }
