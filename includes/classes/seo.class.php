 ...
 */        
        function SEO_URL_INSTALLER(){
                ...
                $this->default_config['SEO_ENABLED'] = array('DEFAULT' => 'true',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Enable SEO URLs?', 'SEO_ENABLED', 'false', 'Enable the SEO URLs?  This is a global setting and will turn them off completely.', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
                $this->default_config['SEO_ADD_MANU'] = array('DEFAULT' => 'false',
                                      'QUERY' => "INSERT INTO `".TABLE_CONFIGURATION."` VALUES (NULL, 'Add manufacturer to product URLs?', 'SEO_ADD_MANU', 'false', 'This setting will add the manufacturer to the beginning of product URLs (i.e. - sony-some-product-p-1.html).', GROUP_INSERT_ID, ".$x.", NOW(), NOW(), NULL, 'tep_cfg_select_option(array(''true'', ''false''),')");
                $x++;
...
//ojp USE_SEO_CACHE_LINKS
                $this->attributes = array('PHP_VERSION' => PHP_VERSION,
                                          ...
                                          'SEO_ADD_MANU' => defined('SEO_ADD_MANU') ? SEO_ADD_MANU : 'false',
 ...
 */        
        function get_product_name($pID){
           ...
           //add manufacturer to url
           $mName = '';
           if ($this->attributes['SEO_ADD_MANU'] == 'true') {
              $mName = $this->get_manufacturer_name_from_product($pID);
                        ...
                        case ($this->uri != $this->real_uri && !$this->not_null($this->path_info)):
                                if (($pStart = strpos($this->uri_parsed['path'], "-p-")) !== FALSE) {
                                    if (($pStop = strpos($this->uri_parsed['path'], ".html")) !== FALSE) {
                                       ...
                                       $pName = $cName . $mName . $this->strip( $result['pName'] );
 ...
 */        
        ...
        function get_manufacturer_name_from_product(&$cID){
