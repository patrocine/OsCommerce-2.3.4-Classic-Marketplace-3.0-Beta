...
define('AR', $customers_id['customers_group_id']);
              ECHO $customers_id['customers_group_id'];
...
function get_products($id_group){
	...
	while ($products_price = tep_db_fetch_array($products_price_query)){
		...
		if(SHOW_MODEL)
         ...
         $customers_porcentage_values = tep_db_query("select * from " . 'products_groups' . " where customers_group_id = '" . 	AR . "' and products_id = '" . 	$products_price['products_id'] . "' ");
       $customers_group = tep_db_fetch_array($customers_porcentage_values);

   
		print "<tr bgcolor=\"".$col."\">".$model."<td width=\"1000\" class=\"productListing-data\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"" . tep_href_link(FILENAME_PRODUCT_INFO, "products_id=" . $products_price['products_id']) . "\">".$products_price['products_name']."</a></td><td width=\"150\" align=\"right\" class=\"productListing-data\">".$currencies->display_price($cell,TAX_INCREASE)."</a></td><td width=\"150\" align=\"right\" class=\"productListing-data\">".$currencies->display_price($customers_id['customers_group_id'],TAX_INCREASE)."</td></tr>";
	}
}
