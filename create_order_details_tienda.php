...
*/
           ...
           echo       $new_value = $HTTP_POST_VARS['new_value'];
         echo       $status = $HTTP_POST_VARS['status'];
...
if ($st_traking){
       ...
       $account['customers_firstname'] = $st_traking ;
       $account['customers_lastname'] = $st_nombre;
       $account['customers_telephone'] = $st_telefono;
...
?></td>
          ...
          <tr>
            <td class="main">&nbsp;<?php echo 'Añadir Porcentage'; ?></td>
            <td class="main">&nbsp;
<?php

   $orders_statuses = array();
  $orders_status_array = array();
  $orders_status_query = tep_db_query("select * from " . 'contabilidad_ciber_tabla_porcentage' . " ORDER BY porcentage_nombre");
  while ($orders_status = tep_db_fetch_array($orders_status_query)) {
    $orders_statuses[] = array('id' => $orders_status['porcentage_nombre'],
                               'text' => $orders_status['porcentage_nombre']);
    $orders_status_array[$orders_status['porcentage_nombre']] = $orders_status['porcentage_nombre'];
  }
  ...
  if ($is_read_only) {
    echo $order->info['porcentage_nombre'];
  } else {                                                      // $order->info['porcentage_nombre']
    echo tep_draw_pull_down_menu('porcentage', $orders_statuses, 0) . '&nbsp;' . 'Porcentage';
  }
?></td>
