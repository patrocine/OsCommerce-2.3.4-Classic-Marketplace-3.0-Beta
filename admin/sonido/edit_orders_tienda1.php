...
// datos del producto
   ...
   $producto['products_price'] =  $groups_['customers_group_price'];
    ...
    $importe = $products_details["final_price"]*$unidades;
   ...
   $porcentage_tienda = tep_db_fetch_array($porcentage_tienda_values);
	...
	///////////////////End bizarro code section
     ...
     $ot_value = $RunningSubTotal * OT_TAX_IVA / 100;
					...
					if ($ot_class == "ot_total") {
					$ot_value = $RunningTotal + ($RunningSubTotal * OT_TAX_IVA / 100);
...
// This calculatiion of Subtotal and Tax is part of the 'add a product' process
  ...
  if  ($customers_porcentage = tep_db_fetch_array($customers_porcentage_values)){
 		...
 		$Query = 'UPDATE ' . TABLE_ORDERS_TOTAL . ' set
				text = "' . $currencies->format($Total, true, $order->info['currency'], $order->info['currency_value']) + $RunningTax . '",
				value = "' . $Total + $RunningTax . '",
  ...
  <p>
   ...
   <input type="submit" name="Submit" value="Proveedor">
          ...
          <?php  }} ?>
            ...
            <a target="_blank" href="invoice_factura_ind_tiket.php<?php ECHO  '?oID=' . $_GET['oID']; ?>"><img src="images/tiket.png" onmouseover="this.src='images/tiket.png';" onmouseout="this.src='images/tiket.png';"/>
            <a target="_blank" href="invoice_factura_ind.php<?php ECHO  '?oID=' . $_GET['oID']; ?>"><img src="images/factura.png" onmouseover="this.src='images/factura.png';" onmouseout="this.src='images/factura.png';"/>
...
echo '	    </td>' . "\n" .
	...
	echo '	    </td>' . "\n" .
		   //  '	    <td class="' . $RowStyle . '" valign="top">' . "<input name='update_products[$orders_products_id][tax]' size='6' value='" . tep_display_tax_value($order->products[$i]['tax']) . "'>" . '</td>' . "\n" .
		      '	    <td class="' . $RowStyle . '" align="left" valign="top">' . "<input name='update_products[$orders_products_id][price]' size='4' value='" . number_format($order->products[$i]['price'], 2, '.', '') . "'>
                    ...
                    <input name='update_products[$orders_products_id][products_price_modificar]' type='checkbox' /" . '<font color="#008080">PVP</font></td></b>' . "\n";
...
require(DIR_WS_INCLUDES . 'application_bottom.php');
   ...
   if  ($donde_esta_c= tep_db_fetch_array($donde_esta_c_values)){
             ...
             $sql_status_update_array = array('donde_esta' => 'A8');
            tep_db_perform('orders_products', $sql_status_update_array, 'update', " products_id='" . (int)$order['products_id'] . "' and orders_id= '" . $oID . "'");


}else{
             $sql_status_update_array = array('donde_esta' => 'A5');
            tep_db_perform('orders_products', $sql_status_update_array, 'update', " products_id='" . (int)$order['products_id'] . "' and orders_id= '" . $oID . "'");
