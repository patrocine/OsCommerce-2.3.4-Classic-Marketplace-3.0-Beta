...
*/
  ...
  class d_total_revenue {
    ...
    function getOutput() {
      ...
      $orders_query = tep_db_query("select date_format(o.date_purchased, '%Y-%m-%d') as dateday, sum(ot.value) as total from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_TOTAL . " ot, administrators a where date_sub(curdate(), interval 60 day) <= o.date_purchased and o.orders_id = ot.orders_id and o.orders_status = a.cobrado and a.admin_groups_id=6 and ot.class = 'ot_total' or
                                                                                                                                                                                                           date_sub(curdate(), interval 60 day) <= o.date_purchased and o.orders_id = ot.orders_id and o.orders_status = a.pagado and a.admin_groups_id=6 and ot.class = 'ot_total' group by dateday");
