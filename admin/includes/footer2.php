...
osCommerce provides no warranty and is redistributable under the <a href="http://www.fsf.org/licenses/gpl.txt" target="_blank">GNU General Public License</a>
 ...
 <p><a href="<?php echo 'index.php?actualizar_cantidades=ok&comprobar_images=ok'; ?>" name="actcan">Actualizar Todo</a></p>
    ...
    <p>Productos que en la web están en negativos, <a href="<?php echo 'index.php?actualizar_cantidades=ok'; ?>" name="actcan">Actualizar Cantidades</a></p>
                 <?php
               ...
               $ayuda_quanty_values = mysql_query("select * from " . TABLE_ORDERS . " where orders_status = '" . 75 . "' order by orders_id DESC");
    $ayuda_quanty = mysql_fetch_array($ayuda_quanty_values);


                $ayuda_producto_values = mysql_query("select * from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.stock_nivel = 6 and p.products_id = pd.products_id and p.time_entradasysalidas <= '" . $limit_stock . "' $sql_products_status order by time_entradasysalidas ASC LIMIT $limit");
   ...
   $resultado =  $entradas_os-$salidas_os;
    ...
    while ($ayuda_producto = mysql_fetch_array($ayuda_producto_values)){
         ...
         ?>
<p style="margin-top: 0; margin-bottom: 0"><a href="<?php echo 'edit_orders_tienda.php?action=edit&rr=&oID='.$ayuda_quanty['orders_id'] . '&codigobarras_negativos='.$ayuda_producto['products_model'] . '&unidades_negativos=' . $tr; ?>">Corregir</a> /
                                           <a target="_blank" href="<?php echo '/product_info.php?products_id='.$ayuda_producto['products_id']; ?>">Web</a> /
<?php echo $ayuda_producto['time_entradasysalidas'] . ' / ' . $ayuda_producto['products_model'] . ' / ' . $ayuda_producto['products_name']; ?> </p>
    ...
    <?php
     ...
     <p>Importante, si quieres que algún producto no aparezca en esta lista solo
tienes que &quot;Inactive&quot; en el catalogo exel.</p>
      <?php

            $comprobar_images_print_values = mysql_query("select * from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and comprobar_images = 1 order by p.products_id  DESC LIMIT 10");
...
//   echo 'ok-';
 ...
 <p style="margin-top: 0; margin-bottom: 0"><a href="<?php echo 'edit_orders_tienda.php?action=edit&rr=&oID='.$ayuda_quanty['orders_id'] . '&eliminar_producto_lista=ok&producto_id=' . $comprobar_images_print['products_id']; ?>">Eliminar</a> /
                                           <a target="_blank" href="<?php echo '/product_info.php?products_id='.$comprobar_images_print['products_id']; ?>">Web</a> /
<?php echo $ayuda_producto['time_entradasysalidas'] . ' / ' . $comprobar_images_print['products_image'] . ' / ' . $comprobar_images_print['products_name']; ?> </p>
    ...
    if(getimagesize(HTTP_SERVER . DIR_WS_CATALOG_IMAGES .$comprobar_images_db['products_image'])) {
                ...
                $sql_status_update_array = array('comprobar_images' => 0,
                                                       'products_status' => 1,);
             tep_db_perform(TABLE_PRODUCTS, $sql_status_update_array, 'update', " products_id= '" . $comprobar_images_db['products_id'] . "' and comprobar_images = 1 and products_image <> 'no-foto.jpg'");
             ...
             tep_db_perform(TABLE_PRODUCTS, $sql_status_update_array, 'update', " products_id= '" . $comprobar_images_db['products_id'] . "' and products_image <> 'no-foto.jpg'");
