    ...
    switch ($action) {
      case 'setflag':
        if ( ($HTTP_GET_VARS['flag'] == '0') || ($HTTP_GET_VARS['flag'] == '1') ) {
          ...
          if (USE_CACHE == 'true') {
            tep_reset_cache_block('marketplace');
            tep_reset_cache_block('also_purchased');
          }
        }

        tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $HTTP_GET_VARS['cPath'] . '&pID=' . $HTTP_GET_VARS['pID']));
        break;
      ...
      case 'update_category':
        ...
        if ($action == 'insert_category') {
          ...
          tep_db_perform('marketplace', $sql_data_array);
          ...
          tep_db_perform('marketplace', $sql_data_array, 'update', "categories_id = '" . (int)$categories_id . "'");
        }
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
         ...
         $categories_status_visible_array = $HTTP_POST_VARS['categories_status_visible'];
          ...
          if ($action == 'insert_category') {
            ...
            tep_db_perform('marketplace_description', $sql_data_array);
            } elseif ($action == 'update_category') {
            tep_db_perform('marketplace_description', $sql_data_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
           tep_db_perform('marketplace_description', $sql_data_suple_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
           tep_db_perform('marketplace_description', $sql_data_status_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
          tep_db_perform('marketplace_description', $sql_data_http_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
          tep_db_perform('marketplace_description', $sql_data_http_mobil_array, 'update', "categories_id = '" . (int)$categories_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
          }
        }
        ...
        if ($categories_image->parse() && $categories_image->save()) {
          tep_db_query("update " . 'marketplace' . " set categories_image = '" . tep_db_input($categories_image->filename) . "' where categories_id = '" . (int)$categories_id . "'");
        }
        ...
        tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $cPath . '&cID=' . $categories_id));
        break;
        ...
        if (isset($HTTP_POST_VARS['categories_id'])) {
          ...
          for ($i=0, $n=sizeof($categories); $i<$n; $i++) {
            ...
            while ($product_ids = tep_db_fetch_array($product_ids_query)) {
              $products[$product_ids['products_id']]['marketplace'][] = $categories[$i]['id'];
            }
          }
          ...
          while (list($key, $value) = each($products)) {
            ...
            for ($i=0, $n=sizeof($value['marketplace']); $i<$n; $i++) {
              $category_ids .= "'" . (int)$value['marketplace'][$i] . "', ";
            }
...
// removing categories can be a lengthy process
        ...
        tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $cPath));
        break;
...
tep_db_query("delete from " . TABLE_PRODUCTS_GROUPS . " where products_id = '" . tep_db_input($product_id) . "' ");
      ...
      case 'move_category_confirm':
        if (isset($HTTP_POST_VARS['categories_id']) && ($HTTP_POST_VARS['categories_id'] != $HTTP_POST_VARS['move_to_category_id'])) {
          ...
          if (in_array($categories_id, $path)) {
            ...
            tep_db_query("update " . 'marketplace' . " set parent_id = '" . (int)$new_parent_id . "', last_modified = now() where categories_id = '" . (int)$categories_id . "'");
            ...
            tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $new_parent_id . '&cID=' . $categories_id));
          }
        }
      ...
      case 'move_product_confirm':
       ...
       // tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $new_parent_id . '&pID=' . $products_id.'&action=new_product'));
        break;
      case 'insert_product':
      case 'update_product':
...
// Insert new large product images
     ...
     if ($HTTP_POST_VARS['pdc_unidades']){
  tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products_id.'&action=new_product'));
...
// datos del producto
  ...
  tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products_id.'&action=new_product'));
       ...
       tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products_id.''));
      ...
      case 'copy_to_confirm':
        ...
        tep_redirect(tep_href_link('marketplace.php', 'cPath=' . $categories_id . '&pID=' . $products_id));
...
$ayuda_reglacategoria = '<p><a href="cpe.php">Ir Crear reglas de caegoria </a> '.'<a class="hastip"  title="' . AYUDA_TEXT_REGLACATEGORIA . '"><b><font size="1" color="#FFFFFF"><span style="background-color: #000000">_?_</span></font></b>';
 ...
 ?>
//--></script>
    <?php echo tep_draw_form('new_product', 'marketplace.php', 'cPath=' . $cPath . (isset($HTTP_GET_VARS['pID']) ? '&pID=' . $HTTP_GET_VARS['pID'] : '') . '&action=' . $form_action, 'post', 'enctype="multipart/form-data"'); ?>
...
<?php
 ...
 tep_db_query("delete from " . 'products_extra_images' . " where products_extra_images_id = '" . $HTTP_GET_VARS['products_extra_images_id'] . "' ");
    ...
    var pagina = '<?php echo 'marketplace.php', '?cPath=' . $HTTP_GET_VARS['cPath'] . '&pID=' . $HTTP_GET_VARS['pID']. '&action=new_product'; ?>';
    var segundos = 0;
...
?>
          ...
          <tr>
            <td class="main"><?php echo 'Precios ' . $ayuda_descuento_cantidad_precio . '<a href="'. 'marketplace.php?del_pdc=' . 'action' . '&pID=' .  $HTTP_GET_VARS['pID'].'&action=new_product' . '"> ->> Eliminar Todo' ; ?></td>
...
<?php
 ...
 ?>
           ...
           <tr>
           <td class="main"><?php echo 'Cantidad: '. $pdc_consulta['pdc_unidades'] . ' precio: '. $pdc_consulta['pdc_price_final'] . '<a href="'. 'marketplace.php?del_pdc_unidad=' . 'action' . '&pID=' .  $HTTP_GET_VARS['pID'].'&action=new_product'.'&pdc_id=' . $pdc_consulta['pdc_id']. '"> ->> Eliminar'; ?></td>
          </tr>
   </tr>
...
<?php
 //  echo tep_draw_form('search', 'marketplace.php', '', 'get');
...
?>
              ...
              <tr>
                <td class="smallText" align="left">
<?php
    echo tep_draw_form('goto', 'marketplace.php', '', 'get');
...
<?php
     ...
     echo '<p><b><a href="' . 'marketplace.php?cPath=' . (int)$current_category_id . 'pID='.$pInfo->products_id.'">VOLVER A LA CATEGORIA</a></b> '.(int)$current_category_id;
  	...
  	tep_db_query ("UPDATE " . TABLE_CONFIGURATION . " SET
       ...
       var pagina = 'marketplace.php<? echo '?cPath=' . $HTTP_GET_VARS['cPath'] . 'pID=' . $HTTP_GET_VARS['pID'];  ?>';
    var segundos = 0;
    ...
    <?php
 	...
 	//tep_db_query ("UPDATE " . TABLE_CONFIGURATION . " SET
      ...
      if ($newproducts = tep_db_fetch_array($newproducts_values)){
       ...
       var pagina = 'marketplace.php<? echo '?cPath=' . $HTTP_GET_VARS['cPath'] . '&action=new_product'.'&ean='.$HTTP_GET_VARS['search'];  ?>';
    var segundos = 0;
    ...
    if (isset($HTTP_GET_VARS['search'])) {
      ...
      $categories_query = tep_db_query("select c.categories_id, cd.categories_name, cd.categories_name_suple, cd.categories_status_visible, cd.categories_name_http, cd.categories_name_http_mobil, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified from " . 'marketplace' . " c, " . 'marketplace_description' . " cd where c.categories_id = cd.categories_id and cd.language_id = '" . (int)$languages_id . "' and cd.categories_name like '%" . tep_db_input($search) . "%' order by c.sort_order, cd.categories_name");
    } else {
...
// Get parent_id for subcategories if search
      ...
      if (isset($cInfo) && is_object($cInfo) && ($categories['categories_id'] == $cInfo->categories_id) ) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('marketplace.php', tep_get_path($categories['categories_id'])) . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('marketplace.php', 'cPath=' . $cPath . '&cID=' . $categories['categories_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link('marketplace.php', tep_get_path($categories['categories_id'])) . '">' . tep_image(DIR_WS_ICONS . 'folder.gif', ICON_FOLDER) . '</a>&nbsp;<strong>' . $categories['categories_name'] . ' | ' . $categories['categories_id'] . '</strong>'; ?></td>
                <td class="dataTableContent" align="center">&nbsp;</td>
                <td class="dataTableContent" align="right"><?php if (isset($cInfo) && is_object($cInfo) && ($categories['categories_id'] == $cInfo->categories_id) ) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('marketplace.php', 'cPath=' . $cPath . '&cID=' . $categories['categories_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
...
// find out the rating average from customer reviews
      ...
      if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id) ) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product_preview') . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '\'">' . "\n";
      }
?>
                <td class="dataTableContent"><?php echo '<a href="' . tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products['products_id'] . '&action=new_product_preview') . '">' . tep_image(DIR_WS_ICONS . 'preview.gif', ICON_PREVIEW) . '</a>&nbsp;' . $products['products_name']; ?></td>
                <td class="dataTableContent" align="center">
<?php
      if ($products['products_status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', IMAGE_ICON_STATUS_GREEN, 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link('marketplace.php', 'action=setflag&flag=0&pID=' . $products['products_id'] . '&cPath=' . $cPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', IMAGE_ICON_STATUS_RED_LIGHT, 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link('marketplace.php', 'action=setflag&flag=1&pID=' . $products['products_id'] . '&cPath=' . $cPath) . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', IMAGE_ICON_STATUS_GREEN_LIGHT, 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', IMAGE_ICON_STATUS_RED, 10, 10);
      }
?></td>
                <td class="dataTableContent" align="right"><?php if (isset($pInfo) && is_object($pInfo) && ($products['products_id'] == $pInfo->products_id)) { echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); } else { echo '<a href="' . tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
              ...
              <tr>
                <td colspan="3"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    ...
                    <td align="right" class="smallText"><?php if (sizeof($cPath_array) > 0) echo tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link('marketplace.php', $cPath_back . 'cID=' . $current_category_id)); if (!isset($HTTP_GET_VARS['search'])) echo tep_draw_button(IMAGE_NEW_CATEGORY, 'plus', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&action=new_category')) . tep_draw_button(IMAGE_NEW_PRODUCT, 'plus', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&action=new_product')); ?>&nbsp;</td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
<?php
    ...
    switch ($action) {
      case 'new_category':
        ...
        $contents = array('form' => tep_draw_form('newcategory', 'marketplace.php', 'action=insert_category&cPath=' . $cPath, 'post', 'enctype="multipart/form-data"'));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath)));
        break;
      case 'edit_category':
        $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_EDIT_CATEGORY . '</strong>');

        $contents = array('form' => tep_draw_form('marketplace', 'marketplace.php', 'action=update_category&cPath=' . $cPath, 'post', 'enctype="multipart/form-data"') . tep_draw_hidden_field('categories_id', $cInfo->categories_id));
       ...
       $contents[] = array('text' => '<br />' . tep_image(DIR_WS_CATALOG_IMAGES . $cInfo->categories_image, $cInfo->categories_name) . '<br />' . DIR_WS_CATALOG_IMAGES . '<br /><strong>' . $cInfo->categories_image . '</strong>');
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_SAVE, 'disk', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id)));
        break;
      case 'delete_category':
        $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_DELETE_CATEGORY . '</strong>');

        $contents = array('form' => tep_draw_form('marketplace', 'marketplace.php', 'action=delete_category_confirm&cPath=' . $cPath) . tep_draw_hidden_field('categories_id', $cInfo->categories_id));
        $contents[] = array('text' => TEXT_DELETE_CATEGORY_INTRO);
        $contents[] = array('text' => '<br /><strong>' . $cInfo->categories_name . '</strong>');
        if ($cInfo->childs_count > 0) $contents[] = array('text' => '<br />' . sprintf(TEXT_DELETE_WARNING_CHILDS, $cInfo->childs_count));
        if ($cInfo->products_count > 0) $contents[] = array('text' => '<br />' . sprintf(TEXT_DELETE_WARNING_PRODUCTS, $cInfo->products_count));
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id)));
        break;
      case 'move_category':
        $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_MOVE_CATEGORY . '</strong>');

        $contents = array('form' => tep_draw_form('marketplace', 'marketplace.php', 'action=move_category_confirm&cPath=' . $cPath) . tep_draw_hidden_field('categories_id', $cInfo->categories_id));
        $contents[] = array('text' => sprintf(TEXT_MOVE_CATEGORIES_INTRO, $cInfo->categories_name));
        $contents[] = array('text' => '<br />' . sprintf(TEXT_MOVE, $cInfo->categories_name) . '<br />' . tep_draw_pull_down_menu('move_to_category_id', tep_get_category_tree(), $current_category_id));
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_MOVE, 'arrow-4', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&cID=' . $cInfo->categories_id)));
        break;
      case 'delete_product':
        $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_DELETE_PRODUCT . '</strong>');

        $contents = array('form' => tep_draw_form('products', 'marketplace.php', 'action=delete_product_confirm&cPath=' . $cPath) . tep_draw_hidden_field('products_id', $pInfo->products_id));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_DELETE, 'trash', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id)));
        break;
      case 'move_product':
        $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_MOVE_PRODUCT . '</strong>');

        $contents = array('form' => tep_draw_form('products', 'marketplace.php', 'action=move_product_confirm&cPath=' . $cPath) . tep_draw_hidden_field('products_id', $pInfo->products_id));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_MOVE, 'arrow-4', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id)));
        break;
      case 'copy_to':
        $heading[] = array('text' => '<strong>' . TEXT_INFO_HEADING_COPY_TO . '</strong>');

        $contents = array('form' => tep_draw_form('copy_to', 'marketplace.php', 'action=copy_to_confirm&cPath=' . $cPath) . tep_draw_hidden_field('products_id', $pInfo->products_id));
        $contents[] = array('text' => TEXT_INFO_COPY_TO_INTRO);
        $contents[] = array('text' => '<br />' . TEXT_INFO_CURRENT_CATEGORIES . '<br /><strong>' . tep_output_generated_category_path($pInfo->products_id, 'product') . '</strong>');
        $contents[] = array('text' => '<br />' . TEXT_CATEGORIES . '<br />' . tep_draw_pull_down_menu('categories_id', tep_get_category_tree(), $current_category_id));
        $contents[] = array('text' => '<br />' . TEXT_HOW_TO_COPY . '<br />' . tep_draw_radio_field('copy_as', 'link', true) . ' ' . TEXT_COPY_AS_LINK . '<br />' . tep_draw_radio_field('copy_as', 'duplicate') . ' ' . TEXT_COPY_AS_DUPLICATE);
        $contents[] = array('align' => 'center', 'text' => '<br />' . tep_draw_button(IMAGE_COPY, 'copy', null, 'primary') . tep_draw_button(IMAGE_CANCEL, 'close', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id)));
        break;
      default:
        if ($rows > 0) {
          if (isset($cInfo) && is_object($cInfo)) { // category info box contents
            ...
            $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('marketplace.php', 'cPath=' . $category_path_string . '&cID=' . $cInfo->categories_id . '&action=edit_category')) . tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('marketplace.php', 'cPath=' . $category_path_string . '&cID=' . $cInfo->categories_id . '&action=delete_category')) . tep_draw_button(IMAGE_MOVE, 'arrow-4', tep_href_link('marketplace.php', 'cPath=' . $category_path_string . '&cID=' . $cInfo->categories_id . '&action=move_category')));
  ...
  $product_values = tep_db_query("select * from " . 'products' . "  where products_id = '" . $pInfo->products_id . "'");
   $product = tep_db_fetch_array($product_values);
            ...
            $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=new_product')) .
            tep_draw_button(IMAGE_DELETE, 'trash', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=delete_product')) .
            tep_draw_button(IMAGE_MOVE, 'arrow-4', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=move_product')) .
            tep_draw_button(IMAGE_COPY_TO, 'copy', tep_href_link('marketplace.php', 'cPath=' . $cPath . '&pID=' . $pInfo->products_id . '&action=copy_to')));
