...
<?php   require(DIR_WS_INCLUDES . 'template_top.php');?>
        ...
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo 'SINCRONIZACION DE CATALOGOS POR PRECIO'; ?></td>
...
$usrfl_size = $HTTP_POST_FILES['usrfl']['size'];
 $usrfl =  $HTTP_POST_FILES['usrfl']['tmp_name'];
   ...
   $lote = $HTTP_GET_VARS['lote'];
      ...
      $link_p = ereg_replace("1.txt", '.txt', $link_p);

      $link_p = ereg_replace(".txt", $lote.'.txt', $link_p);
        ...
        echo "Todos los productos desactivados<br>";
        echo "<br>";
        echo "<br>";
        echo "NO DETENGA NI CIERRE ESTA VENTANA.<br>";
        echo "<br>Pagina Actual: " . $lote;
        echo "<br>";
   ...
   $prove_values = tep_db_query("select * from " . 'proveedor' . " where proveedor_id= '" . $proveedor_id . "'");
     $prove = tep_db_fetch_array($prove_values);
                  ...
                  if ($lote >= 5){
           $sql_status_update_array = array('pagina_actual' => $lote+1);
           tep_db_perform('proveedor', $sql_status_update_array, 'update', " proveedor_id= '" . $proveedor_id . "'");
                              }
              if ($lote == 1){

          //  $sql_status_update_array = array('products_status' => '0');
          // tep_db_perform('products', $sql_status_update_array, 'update', " codigo_proveedor= '" . $proveedor_id . "'");
       ...
       $oldday1 = date("Y-m-d", $time1);
             ...
             $texto_mail .= "\n\n";
             $texto_mail .= "Fecha de ultima Sincronización: " . $oldday1;
             $texto_mail .= "\n\n\n\n";
             $texto_mail .= "¿Que es la sincronizacion?";
             $texto_mail .= "\n\n";
             $texto_mail .= "Se sincronizan los stock de los productos con el proveedor";
             $texto_mail .= "\n\n";
             ...
             $cabezera .=  STORE_OWNER . ' ' . $prove['proveedor_name'] . ' SINCRONIZACION';
...
if (is_uploaded_file($usrfl) && $split==1) {
	...
	echo "You can download your split files in the Tools/Files under /catalog/temp/";
           ...
           $lote=$lote+1;
   if ($lote >= 290){
      // cuando llegue al final reiniciará el contador paginta actual a 0
           $sql_status_update_array = array('pagina_actual' => 1);
           tep_db_perform('proveedor', $sql_status_update_array, 'update', " proveedor_id= '" . $proveedor_id . "'");
           
           
           $sql_status_update_array = array('products_status' => '0',
                                            'control3' => '0'  );        // easypopulate_time3 <= '" . time() . "'
           tep_db_perform('products', $sql_status_update_array, 'update', " control3 <> '" . $prove['control3'] . "' and codigo_proveedor = '" . $proveedor_id . "'");

$control3 = rand(1,9999);

           $sql_status_update_array = array('control3' => $control3, );
           tep_db_perform('proveedor', $sql_status_update_array, 'update', " proveedor_id= '" . $proveedor_id . "'");
            ...
            echo 'TODO EL CATALOGO SE ACTUALIZADO CON EXITO, ESPERE QUE AHORA SE ACTUALIZARÁN LAS NOVEDADES';
             echo 'easypopulate_auto.php?proveedor_id=' . $proveedor_id .'&link_p=' . $prove['link_novedades'] .'&lote=1';
            
?>

                             <script type="text/javascript">

    var pagina = '<?php echo 'easypopulate_auto.php?proveedor_id=' . $proveedor_id .'&link_p=' . $prove['link_novedades'] .'&lote=1'; ?>';
    var segundos = 180000;
   ...
   <?php
    ...
    if ($lote <= 1){
      ...
      echo $tiempo = rand(1,1200000);

?>

                             <script type="text/javascript">

    var pagina = '<?php echo 'easypopulate_sincronizacion_auto.php?' . $_SERVER['QUERY_STRING'].'&lote='.$prove['pagina_actual']; ?>';
    var segundos = <?php echo $tiempo ?>;
...
$time = number_format($time_end - $time_start, 5);
   ...
   if ($time <= 5 ){
              /*
        $tiempo = rand(10000,30000);
}else if ($time <= 20 ){
        $tiempo = rand(20000,50000);
}else if ($time <= 30 ){
        $tiempo = rand(30000,100000);
}else if ($time <= 40 ){
        $tiempo = rand(100000,400000);
}else if ($time <= 60 ){
        $tiempo = rand(120000,500000);
               */
        ...
        $tiempo = rand(10000,180000);
}else if ($time <= 20 ){
        $tiempo = rand(20000,200000);
}else if ($time <= 30 ){
        $tiempo = rand(30000,300000);
}else if ($time <= 40 ){
        $tiempo = rand(100000,400000);
}else if ($time <= 60 ){
        $tiempo = rand(120000,500000);
...
?>
    ...
    var pagina = '<?php echo 'easypopulate_sincronizacion_auto.php?' . $_SERVER['QUERY_STRING'].'&lote='.$lote; ?>';
    var segundos = <?php echo $tiempo ?>;
...
function print_el( $item2 ) {
	echo " | " . substr(strip_tags($item2), 0, 255);
};
...
$oldday2 = date("Y-m-d", $time2);
   ...
   // if ($specials = mysql_fetch_array($specials_values)){     }
     ...
     $wersdf_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_model = '" . $v_products_model . "'");
     $wersdf = mysql_fetch_array($wersdf_values);
...
$oldday1 = date("Y-m-d", $time1);
                                 ...
                                 if  ($vv_new_model){
                                      $v_products_model = $vv_new_model;
                                                        }
                             
                             
                             
                             
     $wersdff_model_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id = '" . $v_products_id . "'");
     $wersdff_model = mysql_fetch_array($wersdff_model_values);
...
$oldday1 = date("Y-m-d", $time1);
     ...
     $orders_values = mysql_query("select * from " . TABLE_ORDERS . " o, " . TABLE_PRODUCTS . " p, " . 'admin' . " a, " . TABLE_ORDERS_PRODUCTS . " op where o.orders_id = op.orders_id and op.products_id = '" . $v_products_id . "' and p.products_id = '" . $v_products_id . "' and o.orders_status = a.abono and op.products_quantity >= 1  ");
   if ($vvvorders = mysql_fetch_array($orders_values)){




     $wersdfs_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id= '" . $v_products_id . "'");
     $wersdfs = mysql_fetch_array($wersdfs_values);
     

    $regla_values = mysql_query("select * from " . 'products_regladeprecios' . " where regladeprecios= '" . $wersdfs['products_regladeprecios'] . "'");
     $regla = mysql_fetch_array($regla_values);


       if ($v_products_price >= $regla['menor'] and $v_products_price <=  $regla['mayor']){




                       if ($regla['porcent_value'] <= 0){
                         $orden = 'ASC';
                   }else{
                     $orden = 'DESC';
               }
       ...
       $v_products_price = $vv_customers_group_price_2 * 9 / 100 + $vv_customers_group_price_2;
        ...
        $sql_status_update_array = array(  'specials_last_modified' => $oldday1,);
            ...
            tep_db_perform(TABLE_SPECIALS, $sql_status_update_array, 'update', " products_id= '" . $v_products_id . "'");
                 ...
                 // ELIMINA LOS DOS ULITMOS NUMEROS DE LA REFERENCIA BINARY.
                if ( $wersdfs['codigo_proveedor'] == 653762 ){
   ...
   $vv_products_cpe = ereg_replace("[0-9]{2}$", "", "$v_products_model");
    ...
    $prove_values = tep_db_query("select * from " . 'proveedor' . " where proveedor_id= '" . $wersdfs['codigo_proveedor'] . "'");
    $prove = tep_db_fetch_array($prove_values);

                                              // echo  $vv_products_cpe;

     // si lo tenemos en stock modifica el PVP Y PVD siempre y cuando estos tenga un valor superior a 0, también modfica el status y la fecha de ultima actualización.
      $sql_data_array = array('products_status' => 1,
                              'control3' => $prove['control3'],
 ...
 // SI NO LO TENEMOS SE MODICA COMO SIEMPRE.
    ...
    if ($regladeprecios= mysql_fetch_array($regladeprecios_values)){
       ...
       $v_products_price = $vv_customers_group_price_2 * 9 / 100 + $vv_customers_group_price_2;
                  ...
                  $modificar_producto_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id = '" . $v_products_id . "' and modificar_producto = 1");
   if  ($modificar_producto = mysql_fetch_array($modificar_producto_values)){
                  ...
                  if ($v_products_price){
        $sql_status_update_array = array('specials_last_modified' => $oldday1,);
                             // 'specials_new_products_price' => $v_products_price,);
                                if ($v_products_price <> '0') $sql_status_update_array['specials_new_products_price'] = $v_products_price;
            ...
            tep_db_perform(TABLE_SPECIALS, $sql_status_update_array, 'update', " products_id= '" . $v_products_id . "'");
                ...
                // si en la lista aparece precio modifica el precio, status y ultima actualizacion del producto
   ...
   $vv_products_cpe = ereg_replace("[0-9]{2}$", "", "$v_products_model");
       ...
       $vv_referencia_fabricante= ereg_replace("[[:space:]]", "", "$vv_referencia_fabricante");
  ...
  $prove_values = tep_db_query("select * from " . 'proveedor' . " where proveedor_id= '" . $wersdfs['codigo_proveedor'] . "'");
    $prove = tep_db_fetch_array($prove_values);
      ...
      $sql_data_array = array('products_status' => 1,
                     ...
                     // if ($vv_referencia_fabricante) $sql_data_array['referencia_fabricante'] = $vv_referencia_fabricante;
                      ...
                      if ($products_price_sin <> 0) $sql_data_array['products_price_sin'] = $products_price_sin;
                      if ($vv_customers_group_price_2 <> 0) $sql_data_array['proveedor_price_general'] = $vv_customers_group_price_2;
                              // if ($modificar_producto['referencia_fabricante'] == '') $sql_data_array['referencia_fabricante'] = $vv_referencia_fabricante;


      tep_db_perform(TABLE_PRODUCTS, $sql_data_array, 'update', "products_id = '" . $v_products_id . "'");
                       // pero si no encuentra precio lo quita de la venta aunque actualiza el precio y la fecha de actualizacion.
   ...
   $vv_products_cpe = ereg_replace("[0-9]{2}$", "", "$v_products_model");
    ...
    if ($vv_nombre){
                   ...
                   $vv_nombre = ereg_replace(",", ", ", $vv_nombre);
      ...
      $sql_data_array = array('products_name' => $vv_nombre,);
      tep_db_perform(TABLE_PRODUCTS_DESCRIPTION, $sql_data_array, 'update', "products_id = '" . $v_products_id . "'");
...
// porcentajes calcular y guardar --------------------------------------------------------------
   ...
   // calcular fecha
     ...
     $fecha = date("Y-m-d", $time);
         ...
         $seguimiento_precios_values = mysql_query("select * from " . 'products_seguimientos_precios' . " where products_id = '" . $v_products_id . "' and fecha >= '" . $fecha_pp . "' group by precio DESC order by fecha DESC");
   while  ($seguimiento_precios = mysql_fetch_array($seguimiento_precios_values)){
    ...
    $precio_medio_total_sales_raw = "select sum(precio) as value, count(*) as precio from " . 'products_seguimientos_precios' . " where products_id = '" . $v_products_id . "' and fecha >= '" . $fecha_pp . "'";
    ...
    // primer precio
         ...
         $precio_antuguo_values = mysql_query("select * from " . 'products_seguimientos_precios' . " where products_id = '" . $v_products_id . "' and fecha >= '" . $fecha_pp . "' order by fecha ASC");
         $precio_antuguo = mysql_fetch_array($precio_antuguo_values);

     // ultimo precio

         $precio_ultimo_values = mysql_query("select * from " . 'products_seguimientos_precios' . " where products_id = '" . $v_products_id . "' and fecha >= '" . $fecha_pp . "' order by fecha DESC");
       ...
       $precio_diferencia_porcent = ($precio_diferencia / $precio_antuguo['precio']) * 100;
            ...
            tep_db_perform('products', $sql_status_update_array, 'update', " products_id = '" . $v_products_id . "'");
...
// fin   porcentajes calcular y guardar    -------------------------------------------------------------
      ...
      $seguimiento_precios_primero_values = mysql_query("select * from " . 'products_seguimientos_precios' . " where products_id = '" . $v_products_id . "'");
   if  ($seguimiento_precios_primero = mysql_fetch_array($seguimiento_precios_primero_values)){

                                                                                                                                                                                           // products_last_modified
      $seguimiento_precios_masdeunavez_values = mysql_query("select * from " . 'products_seguimientos_precios' . " psp, " . 'products' . " p where p.products_id = psp.products_id and p.products_last_modified = '" . $oldday1 . "' and psp.products_id = '" . $v_products_id . "' order by psp.fecha ASC");
   if  ($seguimiento_precios_masdeunavez = mysql_fetch_array($seguimiento_precios_masdeunavez_values)){
 ...
 // NOTIFICACIONES POR CAMBIO DE PRECIO O DE STOCK
     ...
     $p_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id= '" . $v_products_id . "'");
     $p = mysql_fetch_array($p_values);

     $pdes_values = mysql_query("select * from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id= '" . $v_products_id . "'");
     $pdes = mysql_fetch_array($pdes_values);

            $v_products_price = number_format($v_products_price, 2, '.', '');


      $notificaciones_values = mysql_query("select * from " . 'products_notificacion_cambios' . " where products_id = '" . $v_products_id . "' and products_price <> '" . $v_products_price . "' or  products_id = '" . $v_products_id . "' and products_activado <> '" . $p['products_status'] . "'");
   while ($notificaciones = mysql_fetch_array($notificaciones_values)){
  ...
  // fin de alertas de mail.
      ...
      //Borra todos los registros que coincidan con precio y deja la ultima fecha.
         ...
         $seguimiento_precios_values = mysql_query("select * from " . 'products_seguimientos_precios' . " where products_id = '" . $v_products_id . "' group by precio order by fecha DESC");
   while  ($seguimiento_precios = mysql_fetch_array($seguimiento_precios_values)){
    ...
    tep_db_query("delete from " . 'products_seguimientos_precios' . " where precio = '" . $seguimiento_precios['precio'] . "' and products_id = '" . $v_products_id  . "' and id <> '" . $seguimiento_precios['id'] . "'");
     ...
     $wersdff_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_id = '" . $v_products_id . "'");
     $wersdff = mysql_fetch_array($wersdff_values);


      $wersdf_values = mysql_query("select * from " . TABLE_SPECIALS . " where products_id = '" . $v_products_id . "'");
    if ($wersdf = mysql_fetch_array($wersdf_values)){
...
$oldday1 = date("Y-m-d", $time1);
   ...
   $pr_grupo_values = mysql_query("select * from " . 'products_groups' . " where products_id = '" . $v_products_id . "' and customers_group_id = '" . 1 . "'");
if ($pr_grupo = mysql_fetch_array($pr_grupo_values)){
...
// tarifa de precios para el grupo 1
   ...
   $pr_grupo2_values = mysql_query("select * from " . 'products_groups' . " where products_id = '" . $v_products_id . "' and customers_group_id = '" . 2 . "'");
if ($pr_grupo2 = mysql_fetch_array($pr_grupo2_values)){
 ...
 // Tarifas de precios para el grupo 2
   ...
   $vv_products_cpe = ereg_replace("[0-9]{2}$", "", "$v_products_model");
    ...
    // Si el producto esta inactive vacia las casillas mas importantes, para el correcto funcionamiento de la base de datos.
...
//////// attributes stock add end					
 ...
 // http://precioslocos.com/admin/easypopulate_sincronizacion_auto.php?proveedor_id=653766&link_p=http://euroconsolas.com/mcsi/LISTASTOCK-MCSI.txt&lote=1
