 ...
 // si el status de la oferta es uno el PVP se convierte en la oferta.
  ...
  $products_details["final_price"] = $products_details["price"] * $products_details["products_descuento"]/100+$products_details["price"] ;
   ...
   // Insertar el producto en otro pedido ya existente.
    ...
    if ($products_details['numero_nuevo_pedido']){

  $selec_nuevopedido_values = mysql_query("select * from " . TABLE_ORDERS . " o, " . TABLE_ORDERS_PRODUCTS . " op where o.orders_id = op.orders_id and o.orders_id = '" . $products_details['numero_nuevo_pedido'] . "' and op.products_id = '" . (int)$order['products_id'] . "'");
   if ($selec_nuevopedido = mysql_fetch_array($selec_nuevopedido_values)){

   // si existe actualiza la nueva cantidad.


           $sql_status_update_array = array('products_quantity' => $selec_nuevopedido["products_quantity"]+$products_details['unidades_nuevo_pedido'], );
            tep_db_perform(TABLE_ORDERS_PRODUCTS, $sql_status_update_array, 'update', " products_id= '" . (int)$order['products_id'] . "' and orders_id = '" . $products_details['numero_nuevo_pedido'] . "'");
...
// precio total del pedido actual
   $precioactual_total_values = mysql_query("select * from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . $products_details['numero_nuevo_pedido'] . "' and class = '" . 'ot_total' . "'");
   $precioactual_total = mysql_fetch_array($precioactual_total_values);

// precio total del pedido actual
   $precioactual_subtotal_values = mysql_query("select * from " . TABLE_ORDERS_TOTAL . " where orders_id = '" . $products_details['numero_nuevo_pedido'] . "' and class = '" . 'ot_subtotal' . "'");
...
// datos del producto
   ...
   $producto = mysql_fetch_array($producto_values);
    ...
    $importe = $producto['products_price']*$products_details['unidades_nuevo_pedido'];

                  // ot_total
           $sql_status_update_array = array('value' => $precioactual_total['value']+$importe,
                                            'text' => $precioactual_total['value']+$importe.'€', );
            tep_db_perform(TABLE_ORDERS_TOTAL, $sql_status_update_array, 'update', " orders_id = '" . $products_details['numero_nuevo_pedido'] . "' and class = '" . 'ot_total' . "'");


                  // ot_subtotal
           $sql_status_update_array = array('value' => $precioactual_subtotal['value']+$importe,
                                            'text' => $precioactual_subtotal['value']+$importe.'€', );
            tep_db_perform(TABLE_ORDERS_TOTAL, $sql_status_update_array, 'update', " orders_id = '" . $products_details['numero_nuevo_pedido'] . "' and class = '" . 'ot_subtotal' . "'");
   ...
   require('includes/proveedores_precios.php');
           ...
           $Query = "INSERT INTO " . TABLE_ORDERS_PRODUCTS . " set
              orders_id = '" . $products_details['numero_nuevo_pedido'] . "',
              products_id = '" . $codigobarras_c['products_id'] . "',
              products_model = '" . $codigobarras_c['products_model'] . "',
              products_name = '" . $codigobarras_c['products_name'] . "',
              products_price = '". $codigobarras_c['products_price'] . "',
              ...
              final_price_tienda = '" . $ganancias_tiendas . "',
              products_tax = '" . $ProductsTax . "',
              products_quantity = '" . $products_details['unidades_nuevo_pedido'] . "'";
              tep_db_query($Query);
              $new_product_id = tep_db_insert_id();
...
echo    '	<td class="' . $RowStyle . '" valign="top">' . "<input name='update_products[$orders_products_id][numero_nuevo_pedido]' size='6' value='" . $order->products[$i]['numero_nuevo_pedido'] . "'>";
echo    '	<td class="' . $RowStyle . '" valign="top">' . "<input name='update_products[$orders_products_id][unidades_nuevo_pedido]' size='2' value='" . $order->products[$i]['unidades_nuevo_pedido'] . "'>";
                                                 }
