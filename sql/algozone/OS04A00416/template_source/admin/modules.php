...
*/
  ...
  #A BOF
  $install = (isset($HTTP_GET_VARS['install']) ? $HTTP_GET_VARS['install'] : '');
  $file_extension = substr($PHP_SELF, strrpos($PHP_SELF, '.'));
  $az_boxes = glob($module_directory."az_*".$file_extension);
  $is_az_template = count( $az_boxes );
  if ($set == 'boxes' && tep_not_null($install) && $install == 'az' && $is_az_template ) {

	  $modules_installed = constant($module_key);
      if ( !empty($modules_installed) ) {
          $modules_installed = explode(';', $modules_installed);

          //remove standard boxes
          foreach ( $modules_installed as $filename ) {
              $class = basename($filename, $file_extension);
      ...
      tep_redirect(tep_href_link(FILENAME_MODULES, 'set=boxes' ));
  }
  #AZ EOF
...
<?php
  if (isset($HTTP_GET_VARS['list'])) {
    ...
    #AZ BOF
    echo '            <td class="smallText" align="right">' . tep_draw_button(IMAGE_MODULE_INSTALL . ' (' . $new_modules_counter . ')', 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&list=new'));
    if ( $set == 'boxes' && $is_az_template )
        echo tep_draw_button('Install template layout', 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&install=az'), '', array('params' => ' onclick="return confirm(\'Your current boxes will be disabled. Are you agree?\'); " ') );
    echo '</td>';
    #AZ EOF
  }
?>
