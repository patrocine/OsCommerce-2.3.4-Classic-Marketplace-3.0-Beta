...
// needs to be included earlier to set the success message in the messageStack
  require(DIR_WS_LANGUAGES . $language . '/' . str_replace("mobile_", "", FILENAME_ADDRESS_BOOK_PROCESS));
  ...
  if (isset($HTTP_GET_VARS['action']) && ($HTTP_GET_VARS['action'] == 'deleteconfirm') && isset($HTTP_GET_VARS['delete']) && is_numeric($HTTP_GET_VARS['delete'])) {
    ...
    tep_redirect(tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK, '', 'SSL'));
  }
...
// reregister session variables
  ...
  if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
    $breadcrumb->add(NAVBAR_TITLE_MODIFY_ENTRY, tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK_PROCESS, 'edit=' . $HTTP_GET_VARS['edit'], 'SSL'));
  } elseif (isset($HTTP_GET_VARS['delete']) && is_numeric($HTTP_GET_VARS['delete'])) {
    $breadcrumb->add(NAVBAR_TITLE_DELETE_ENTRY, tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK_PROCESS, 'delete=' . $HTTP_GET_VARS['delete'], 'SSL'));
  } else {
    $breadcrumb->add(NAVBAR_TITLE_ADD_ENTRY, tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK_PROCESS, '', 'SSL'));
  }
require(DIR_MOBILE_INCLUDES . 'header.php');
$headerTitle->write($headerTitleText);
?>
<div id="iphone_content">
<?php if (!isset($HTTP_GET_VARS['delete'])) echo tep_draw_form('addressbook', tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK_PROCESS, (isset($HTTP_GET_VARS['edit']) ? 'edit=' . $HTTP_GET_VARS['edit'] : ''), 'SSL'), 'post', 'onSubmit="return check_form(addressbook);"'); ?>
...
?>
<div id="accthistpl">
	  <strong><?php echo DELETE_ADDRESS_DESCRIPTION; ?><br><br>
	  <?php echo SELECTED_ADDRESS . ':'; ?></strong><br>
	  <?php echo tep_address_label($customer_id, $HTTP_GET_VARS['delete'], true, ' ', '<br>'); ?><br><br>
</div>	  
<div id="accthistpl">
	<div id="bouton" style="text-align:left;">
	<?php echo '<a href="' . tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK, '', 'SSL') . '">' . tep_mobile_button(IMAGE_BUTTON_BACK) . '</a>' . 
		   '<span style="float:right;"><a href="' . tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK_PROCESS, 'delete=' . $HTTP_GET_VARS['delete'] . '&action=deleteconfirm', 'SSL') . '">' . tep_mobile_button(IMAGE_BUTTON_DELETE) . '</a></span>';?>
	</div>
</div>
...
?>
      <?php include(DIR_MOBILE_MODULES . 'mobile_address_book_details.php'); ?>
<?php
    if (isset($HTTP_GET_VARS['edit']) && is_numeric($HTTP_GET_VARS['edit'])) {
?>
<div id="accthistpl">
	<div id="bouton">
	<?php echo tep_draw_hidden_field('action', 'update') . tep_draw_hidden_field('edit', $HTTP_GET_VARS['edit']) . tep_mobile_button(IMAGE_BUTTON_UPDATE); ?>
	</div>
</div>
<?php
    } else {
      if (sizeof($navigation->snapshot) > 0) {
        ...
        $back_link = tep_mobile_link(FILENAME_MOBILE_ADDRESS_BOOK, '', 'SSL');
      }
?>
<div id="accthistpl">
	<div id="bouton">
	<?php echo tep_draw_hidden_field('action', 'process') . tep_draw_hidden_field('edit', $HTTP_GET_VARS['edit']) . tep_mobile_button(IMAGE_BUTTON_CONTINUE); ?>
	</div>
</div>
<?php
    }
