...
<h1><?php echo 'EMPRESA 3.0 OsOcommerce'; ?></h1>      <?php
...
<?php
  ...
  //$affiliate_sales_raw = "select op.products_name, op.orders_id from " . TABLE_ORDERS . " o, " . TABLE_PRODUCTS . " p, " . 'admin' . " a left join " . TABLE_ORDERS_PRODUCTS . " op on op.orders_id = o.orders_id where op.products_id = p.products_id and o.orders_status = '" . $entregas_stock . "'";
		...
		$affiliate_sales_raw = "select p.products_id,
                                                      ...
                                                      p.products_status,
                                                      p.codigo_proveedor,
                                                      p.proveedor_price from " . TABLE_PRODUCTS . " p,  " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and p.time_entradasysalidas < p.products_stock_min and p.products_stock_min <> 0 and p.codigo_proveedor = '" . 	653801 . "'";



    $affiliate_sales_split = new splitPageResults($HTTP_GET_VARS['page'], 1000, $affiliate_sales_raw, $affiliate_sales_numrows);
...
?></td>
     ...
     <td class="dataTableContent" align="center"><?php
                                               ...
                                               echo $affiliate_sales['time_entradasysalidas'] . '| ' . $affiliate_sales['time_mercancia_entregado_procesando'] . '| ' . $affiliate_sales['products_stock_min'] . '| ' . $affiliate_sales['products_stock_obs'];
