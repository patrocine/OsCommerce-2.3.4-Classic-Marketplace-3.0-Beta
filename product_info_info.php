...
<div>
  ...
  echo '<p><font style="font-size: 24pt; font-weight: 700">'.$products_price.'</font></p>';
                   }
	...
	<tr>
		...
		<td><font color="#FF0000">  <?php ECHO number_format($precio_diferencia_porcent, 2, '.', '').'%'; ?></font> </td>
           ...
           <?php
                                                 ...
                                                 if (PERMISO_REGLADEPRECIOS_PRODUCTSINFO == 'True' and $product_info['modificar_categoria_rdc'] == 0){
...
// Points/Rewards system V2.1beta EOF
 ...
 $twitter_text =  number_format($precio_diferencia_porcent, 2, '.', '') . '% ' . number_format($product_info['products_price'], 2, '.', '') . '€ ' . $product_info['products_name'] . ' ' .      . ' ' . $product_descripcion_twitter . ' ' . $pi_twiiter_image;

 }else{
 $twitter_text =  '+' . number_format($precio_diferencia_porcent, 2, '.', '') . '% ' . number_format($product_info['products_price'], 2, '.', '') . '€ ' . $product_info['products_name'] . ' ' . HASTAG_TWITTER . ' ' . $product_descripcion_twitter . ' ' . $pi_twiiter_image;
...
<?php
     ...
     ?>                                          <p>&nbsp;</p>
          ...
          <p style="margin-top: 0; margin-bottom: 0"><b>Deje Su Mail y le avisamos cuando el producto cambie de precio (Solo se usa para la alerta)</b></p>
