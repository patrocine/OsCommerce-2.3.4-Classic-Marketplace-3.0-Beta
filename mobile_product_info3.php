...
require(DIR_MOBILE_INCLUDES . 'header.php');
 ...
 echo $pro_mo = $_GET['products_id'] ;
 

      
      
  $product_check_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "' or p.products_status = '1'  and pd.products_id = p.products_id and  p.products_model = '" . $pro_mo . "' and pd.language_id = '" . (int)$languages_id . "'");
...
<?php
    ...
    $product_info_query = tep_db_query("select opcion_1, opcion_1_1,
                                               ...
                                               p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_status = '1' and p.products_id = '" . (int)$HTTP_GET_VARS['products_id'] . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "' or p.products_status = '1' and p.products_model = '" . $pro_mo . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$languages_id . "'");
    $product_info = tep_db_fetch_array($product_info_query);



     $producto_values = mysql_query("select * from " . TABLE_PRODUCTS . " where products_model = '" . $pro_mo . "'");
    if ($producto= mysql_fetch_array($producto_values)){
      ...
      $HTTP_GET_VARS['products_id'] = $producto['products_id'];
             ...
             ECHO $products_id_stock = $HTTP_GET_VARS['products_id'];
